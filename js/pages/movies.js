...
const renderPlayer = (movie) => {
    if (!movie) return;
    document.title = `${movie.name || 'Watch'} - StreamX`;
    ...
    // Info
    if (title) title.innerText = movie.name || movie.title || 'Untitled';
    if (date) date.innerText = movie.year || movie.date || 'N/A';
    if (rating) rating.innerText = movie.imdb || movie.rating || 'N/A';
    if (gen) gen.innerText = movie.genre || 'Genre';
    if (desc) desc.innerText = movie.description || "No description available.";

    // Video
    if (movie.type === 'series' && movie.seasons) {
        setupSeriesUI(movie);
        const firstSeason = Object.keys(movie.seasons)[0];
        if (firstSeason && movie.seasons[firstSeason] && movie.seasons[firstSeason].length > 0) {
